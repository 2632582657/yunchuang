	...
	<div class="comNumPreview"
		...
		<div  ref="headpro"
			...
			<div class="grid-content bg-purple-light rout" style="text-align:left;padding:0 19px;"
				...
				<div class="actived" @click="()=> {$router.push({name:'prductInfoTab'})}">
					产品展示
				</div>
				<div @click="()=> {$router.push({name:'jobInfoTab'})}">
					<!-- <div @click="handleData"> -->
					职位招聘
				</div>
			</div>
		...
		<div class="product-show maincontentpro"
			 style="padding:0 19px;height:635px;overflow-y:scroll;">
			<template v-for="(item, index) in getInfo">

			<div class="product-column"
				 style="cursor:pointer;overflow:hidden;border-bottom:1px solid #EDEFF2;padding:19px 0 20px;"
				 @click.stop="hrefTopro(item,index)">
				<el-container>
					...
					<el-main style="padding-left:15px;background:#fff;padding-top:0;">
						<div style="height:100%;;position: relative;">
							<p style="display:block;text-align:left;color:#303133;margin-bottom:20px;font-size:18px;">
								{{item.listName}}</p>
							<p style="text-align:left;color:#606266;font-size:16px;" class="content" v-html="item.content">
								{{item.content}}
							</p>
...
<script>
	...
	export default {
		...
		methods: {
			scrollTip() {
				if (!this.scroll) {
					this.mainScrollpro = new BScroll('.maincontentpro', {
						scrollY: true,
						click: true,
						probeType: 3,
			...
			hrefTopro(item, index) {
				console.log(item)
				this.$router.push({name: 'productInfo',query:{id:item.id,i:index, length:this.getInfo.length}})
			},
...
<style lang="scss">
	...
	.comNumPreview::-webkit-scrollbar, .product-show::-webkit-scrollbar {
		display: none;
	}
	.comNumPreview::-webkit-scrollbar, .product-show::-webkit-scrollbar {
		width: 0 !important
	}
	.comNumPreview, .product-show {
