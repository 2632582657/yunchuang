<template>
	<div class="sendNotice-container">
		<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
			...
			<el-form-item label="附件:" prop="fileListAttachment">
				<el-upload
					class="upload-demo"
				...
				>
					...
					<div slot="tip" class="el-upload__tip">支持.doc,.docx,.pdf,.xls,.xlsx,.ppt,.pptx,.zip类型文件，单文件10M以内
					</div>
				</el-upload>
			</el-form-item>
			...
			<div class="comRuleList-movetabel" v-show="transferShow">
				...
				<div style="overflow:hidden;">
					<div class="movetabel-content">
						...
						<template>
							<div
								style="width:400px;height:450px;overflow:scroll;border:1px solid #bbb;border-radius:10px;padding:20px 10px;text-align:center;box-sizing:border-box;">
								<el-input
					...
					<div class="movetabel-class">
						<p>已选择</p>
						<div
							style="border:1px solid 	#bbb;width:400px;height:450px;border-radius:10px;padding:20px 	10px;box-sizing:border-box">
							<template v-for="(item,index) in choice">
								<div class="el-icon-user"
									 style="width:100%;height:50px;line-height:50px;overflow:hidden;font-size:18px;background:pink;margin-bottom:10px;">
									{{item.label}}({{item.la}})
									<span class="el-icon-close" style="float:right;line-height:50px;font-size:18px;"
										  @click="del(item,index)"></span>
								</div>
							</template>
						</div>
					</div>
			...
			<el-form-item class="sendNotice-confirm-btn">
				<el-button type="primary" @click="submitForm('ruleForm')">发布</el-button>
				<el-button type="primary" @click="()=>{previewModuleFlag=true}">预览</el-button>
				<!-- <el-button @click="resetForm('ruleForm')">重置</el-button> -->
			</el-form-item>
		</el-form>
		<div class="preview-module" v-show="previewModuleFlag">
			<h1>
				<span>公告预览</span>
				<span class="el-icon-close close" @click.stop="()=>{previewModuleFlag=false}"></span>
			</h1>
			<div class="previewModule-container">
				<p class="title">{{ruleForm.name}}关于修订考勤管理制度（暂行）的通知</p>
				<p class="info">
					<span>{{creatTime}}2019/08/05 12:55 </span>
					<span>{{ruleForm.valued}}公告所属分类名称</span>
				</p>
				<!--				<div v-html="ruleForm.content" class="content">-->
				<div class="content">
					{{ruleForm.content}}
					1、考勤周期：2017年3月工资结算的考勤统计周期为3月1日—3月31日；</br>
					2、考勤数据上报：工作日必须在所属区域公司范围内IP地址登陆或移动签到，直接前往客户处未能在公司范围内签到确认出勤的，请务必提交外出出差流程，签到记录仅以规定范围可识别的数据作为统计依据。</br>
					3、考勤报表查询方式：请大家在系统中进入报表菜单——人事报表——考勤相关——考勤报表，查看自己的考勤记录。</br>
					...
					</br>5、考勤统计时效：外出及请假流程的补交最晚截至到4月7日12：00，请上级及时审批，以便我们统计，逾期未及时提交流
					<!--				</div>-->
				</div>
				<p class="footer">{{pSection}}签发部门：发布人所在部门</p>
			</div>
...
<script>
	...
	export default {
		data() {
			return {
				previewModuleFlag: false,//预览开关
				pSection: '',//存储发布人所在部门。从本地获取
				fileListAttachment: [],//附件
				creatTime: '',//预览中的时间===点击预览的时候要创建时间格式化
				filterText: '',//存储筛选出的信息
				data: [{
					id: 1,
					label: '一级 1',
		...
		methods: {
			...
			commit() {
				...
				this.ruleForm.input = this.ruleForm.input.substring(this.ruleForm.input.length, this.ruleForm.input.length - 1);
	...
	*::-webkit-scrollbar {
	...
	.sendNotice-container {
		.preview-module {
			...
			.previewModule-container {
				padding: 37px 41px 40px 45px;
