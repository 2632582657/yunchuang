		...
		<div class="summary_content clear">
			<div class="summary_search">
				...
				<el-form class="clear" style="margin:15px 0 0px 0">
                    ...
                    <el-form-item class="fl" style="width: 100px;">
						<div style="color:#2AA1E1;cursor:pointer" @click="handleAllSign">全部标记已读</div>
					</el-form-item>
                    <el-form-item class="fl" style="width: 100px;">
						<el-checkbox v-model="checked" @change="getApprovalData">仅显示未读</el-checkbox>
					</el-form-item>
				</el-form>
			</div>
                    ...
                    <el-table-column align="center" label="审批标题" prop="approveName">
						<div slot-scope="scope" class="clear">
                            <div class="approval_isRead">
                                ...
                                <div class="isRead_position" v-if="scope.row.isRead == 0"></div>
                            </div>
                        </div>
                    </el-table-column>
...
<script>
	...
	export default {
		...
		data() {
            ...
            sumitDate.push(rs);
            let completeDate = []
            completeDate.push(nowMonthTime);
            completeDate.push(rs);
			return {
				pickerOptions: {
					shortcuts: [{
						text: '今天',
		...
		methods: {
            ...
            getApprovalData () {
                this.$http({
                    method:'POST',
                    url:'oaManager/v1/feign/oa/approve/queryRecipientMyApprove',
                    data:{
                        ...
                        recipientId:this.userDetail.id,
            ...
            handleApprvolDetial (obj) {
				this.$http({
					method:'GET',
					url:`/oaManager/v1/feign/oa/approve/queryApproveInfoAndEditReadStatus/${this.userDetail.id}/${obj.approveTypeId}/${obj.approveChildrenId}`
				}).then(res => {
					if(res.data.code == 200) {
            ...
            handleAllSign () {
                this.$http({
                    method:'GET',
                    url:`oaManager/v1/feign/oa/approve/editRecipientsReadStatus/${this.userDetail.id}`
                }).then(res => {
                    if (res.data.code == 200) {
                        this.$message({
                            type:'success',
...
<style lang="scss">
    #copyApproval .el-table .warning-row {
        background: #F6F6FA;
    }
    #copyApproval  .el-table .success-row {
        background: #fff;
    }
	#copyApproval .sender {
	...
	#copyApproval .daterangeData {
		width: 340px;
	}
    #copyApproval .el-table__row:hover{
        cursor: pointer;
    }
    #copyApproval .awaitApproval_form {
		border-bottom: 1px solid #E6E8ED;
		.el-form-item {
			margin-bottom: 0px;
		}
	}
	#copyApproval .summary_content_heade span {
		color: red;
		margin-right: 4px;
	}

	#copyApproval .summary_content_heade {
		color: #606266;
		font-size: 14px;
	}
    #copyApproval .approval_isRead {
        position: relative;
    }
    #copyApproval .isRead_position {
        position: absolute;
        height: 10px;
        width: 10px;
        left: 0px;
	...
	#copyApproval .summary_content {
		width: 97%;
		background-color: white;
		padding:20px;
	}

	#copyApproval .back_ground {
		border-bottom: 5px solid #4278BE;
		color: #4278BE !important;
	}

	#copyApproval .btn_from_tab li {
		color: #BFC2CC;
		font-size: 18px;
		width: 118px;
		height: 46px;
