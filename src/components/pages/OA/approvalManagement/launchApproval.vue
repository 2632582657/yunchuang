	...
	export default {
		...
		created() {
			...
			if(this.$route.params.dataObj){
				this.handleApprvolDetial(this.$route.params.dataObj)
			}
		},
		methods: {
            //表格隔行变色
            ...
            getApprovalData () {
                this.$http({
                    method:'POST',
                    url:'oaManager/v1/feign/oa/approve/queryIInitiateApprove',
                    data:{
...
<style lang="scss">
    #launchApproval .el-table .warning-row {
        background: #F6F6FA;
    }
    #launchApproval  .el-table .success-row {
        background: #fff;
    }
	#launchApproval .sender {
	...
	#launchApproval .daterangeData {
		width: 340px;
	}
	#launchApproval .summary_content_heade span {
		color: red;
		margin-right: 4px;
	}
    #launchApproval .awaitApproval_form {
		border-bottom: 1px solid #E6E8ED;
		.el-form-item {
			margin-bottom: 0px;
		}
	}
	#launchApproval .summary_content_heade {
		color: #606266;
		font-size: 14px;
	}
    #launchApproval .el-table__row:hover{
        cursor: pointer;
    }
	#launchApproval .summary_content {
		width: 97%;
		background-color: white;
		padding:20px;
	}

	#launchApproval .back_ground {
		border-bottom: 5px solid #4278BE;
		color: #4278BE !important;
	}

	#launchApproval .btn_from_tab li {
		color: #BFC2CC;
		font-size: 18px;
		width: 118px;
		height: 46px;
