                ...
                <el-table
                    ...
                    <el-table-column align="center" label="审批标题" prop="approveName">
						<div slot-scope="scope" class="clear">
                            <img :src="scope.row.iconUrl" alt="" width="45px" height="45px" style="border-radius:3px;" class="fl" v-if="scope.row.iconUrl">
							<img src="./images/icon_head.png" alt="" width="45px" height="45px" style="border-radius:3px;" class="fl" v-else>
                    ...
                    <el-table-column align="center" label="状态" prop="displayStatus">
                        <template slot-scope="scope">
                            <div style="color:#4FBE96" v-if="scope.row.approveStatus == 3">{{scope.row.displayStatus}}</div>
	...
	export default {
		...
		methods: {
            ...
            getApprovalData () {
                this.$http({
                    method:'POST',
                    url:'oaManager/v1/feign/oa/approve/queryWaitMyApprove',
                    data:{
                    ...
                    if (res.data.code == 200) {
                        ...
                        this.awaitApprovalData = res.data.data.list
                    }else if (res.data.code == 10021) {
                        this.awaitApprovalData = []
                    }
                })
            },
            // 查看审批详情
...
<style lang="scss">
    #awaitApproval .el-table .warning-row {
        background: #F6F6FA;
    }
    #awaitApproval  .el-table .success-row {
        background: #fff;
    }
	#awaitApproval .sender {
	...
	#awaitApproval .awaitApproval_form {
		border-bottom: 1px solid #E6E8ED;
		.el-form-item {
			margin-bottom: 0px;
		}
	}
	#awaitApproval .el-table__row:hover{
        cursor: pointer;
    }
	#awaitApproval .daterangeData {
		width: 342px;
	}
	#awaitApproval .summary_content_heade span {
		color: red;
		margin-right: 4px;
	}

	#awaitApproval .summary_content_heade {
		color: #606266;
		font-size: 14px;
	}

	#awaitApproval .summary_content {
		width: 97%;
		background-color: white;
		padding:20px;
	}

	#awaitApproval .back_ground {
		border-bottom: 5px solid #4278BE;
		color: #4278BE !important;
	}

	#awaitApproval .btn_from_tab li {
		color: #BFC2CC;
		font-size: 18px;
		width: 118px;
		height: 46px;
