<!--@description: 申请加入企业页面
	@author: 王跃飞
	@update: 王跃飞 (2019-09-17 17:45)-->
<template>
	<div class="agreeInvita-container" ref="mark">
		<img src="../images/tongyi@2x.png" style="width: 44px;height: 44px;position: absolute;top:-55px;margin-left: -69px;z-index: 99;">
		<template v-for="(item,index) in invitationList">
			<div class="agreeInvita-item">
				<div class="agreeInvita-time">
					<span>{{item.createTime}}</span>
				</div>
				<div class="agreeInvita-content">
		...
		methods: {
			getJoinComDate(pageNum) {
				this.$http({
					...
					data: {
						msgType: JSON.stringify(5),
						pageNum: JSON.stringify(pageNum),
					},
					headers: {
						userId: JSON.parse(this.adminInfo.id),
					}
				}).then(res => {
